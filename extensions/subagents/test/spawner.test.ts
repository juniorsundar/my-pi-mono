...
import type { ActivityFeedOutput } from "../src/activity-feed-formatter.js";
...
`,
  ...
  it("prunes oldest completed workspaces on spawn while keeping uncompleted ones", async () => {
    const workDir = makeWorkDir();
    const agentsDir = makeAgentsDir();
    writeAgentDef(agentsDir, "scout");

    // Seed subagents root: 21 completed (so 1 oldest will be evicted) + 1 uncompleted.
    const store = new WorkspaceStore(join(workDir, ".pi", "subagents"));
    for (let i = 0; i < 21; i++) {
      const ws = store.create(`old-${String(i).padStart(2, "0")}`);
      ws.writeOutput(`output ${i}`);
    }
    store.create("pending").writeTask("no output yet");

    const binDir = join(workDir, "bin");
    mkdirSync(binDir);
    const piPath = writeFakePi(binDir, `#!/usr/bin/env bash
cat <<'EOF'
{"type":"agent_start"}
...
`);
    ...
    const finalStore = new WorkspaceStore(join(workDir, ".pi", "subagents"));
    const remainingIds = finalStore.list().map((ws) => ws.agentId).sort();

    expect(result.agentId).toBe("new-run");
    expect(remainingIds).not.toContain("old-00"); // oldest completed evicted
    expect(remainingIds).toContain("pending"); // uncompleted untouched
    expect(remainingIds).toContain("new-run"); // current run kept
    expect(remainingIds).toHaveLength(22); // 20 retained old + pending + new-run
  }, 10000);
});
